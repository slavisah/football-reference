 ...
 * PDFs while building this function (see docs/PROJECT_STATUS.md's
 * hundred-and-forty-seventh-intensive-run entry): every existing
 * *site-relative* link one of them carries (e.g. a team-profile link built
 * with `withBase()`) resolved to a dead
 * `http://localhost:4399/football-reference/...` URI in the shipped file - a
 * real bug in every one of the 700 PDFs' *other* internal links at the time,
 * fixed in the hundred-and-forty-eighth intensive run (see that entry). A
