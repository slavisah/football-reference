...
/**
 * Filename (no directory) of one edition's downloadable PDF under
 * public/downloads/ - the `edition-<family>-<slug>[-hr]` naming convention
 * scripts/pdf-pages.mjs's `EDITION_PDF_SOURCES` map and
 * src/pages/edition-index.json.ts's `entriesFor()` both already use.
 * `family` is one of those same keys (e.g. "world-cup",
 * "golden-boot-world-cup"); `slug` is this edition's own `EditionProfile.slug`
 * (or a neighbour's, for the pager below) - already host-disambiguated where
 * needed (Copa América's two 1959 tournaments), since it's used exactly as
 * given.
 */
export function editionPdfFileName(family: string, slug: string, locale: 'en' | 'hr'): string {
  return `edition-${family}-${slug}${locale === 'hr' ? '-hr' : ''}.pdf`;
}

/**
 * Absolute production URL (scheme + host + base path) of that same PDF -
 * needed for `EditionView.astro`'s print-only PDF-to-PDF cross-reference
 * pager (the "Previous/Next edition" pair pointing at the *sibling edition's
 * own downloadable PDF*, for a reader working from a folder of downloaded
 * PDFs rather than the live site).
 *
 * Deliberately NOT a `withBase()`-only, site-relative href: Chromium's
 * `page.pdf()` (scripts/generate-pdfs.mjs) bakes a link annotation from
 * whatever the anchor's already-*resolved* href is. A relative href is
 * resolved by the browser against the page's *current* origin - during PDF
 * generation, that is the script's own `http://localhost:4399/...` preview
 * server, not the real published site (the same `class="url"` trap that
 * file's own footer-template comment already documents for a different
 * element). Confirmed empirically against this repo's own already-shipped
 * PDFs while building this function (see docs/ROADMAP.md's
 * hundred-and-forty-seventh entry): every existing *site-relative* link one
 * of them carries (e.g. a team-profile link built with `withBase()`) resolves
 * to a dead `http://localhost:4399/football-reference/...` URI in the
 * shipped file - a real, previously-undocumented bug in every one of the 700
 * PDFs' *other* internal links, left for a future pass rather than fixed here
 * (out of this run's scope; see that same entry). A literal absolute URL,
 * by contrast, is baked into the static HTML unchanged at Astro's *build*
 * time (`site`/`fallback` below are `Astro.site`/`Astro.url`, resolved when
 * the page is generated by `astro build`, hours or days before
 * `generate-pdfs.mjs` ever runs `astro preview`) - so it survives into the
 * printed PDF exactly as written, regardless of what origin Chromium later
 * happens to navigate to.
 */
export function editionPdfDownloadUrl(
  site: URL | undefined,
  fallback: URL,
  family: string,
  slug: string,
  ...
  return new URL(withBase(`/downloads/${editionPdfFileName(family, slug, locale)}`), site ?? fallback).toString();
}
