...
- **Copa América's 1979 winning-captain gap finally resolved**: closed
  2026-09-05 (sixty-seventh intensive run) - a standing health check first
  (`pnpm install`, `pnpm outdated` found only the still-blocked `typescript`
  7 entry, `pnpm lint` clean across all files). Re-checked the sixty-sixth
  run's two open leads before searching for anything new: UEFA Nations
  League Team of the Tournament for 2021/2023/2025 stays blocked (a fresh
  `WebFetch` attempt against a UEFA.com recap article this run confirmed the
  egress proxy still rejects `www.uefa.com` outright, not just Wikipedia).
  Took a genuinely different approach to Copa América's own long-standing
  1979 captain gap instead of repeating the general-English-source searches
  every prior attempt had used: targeted Spanish-language `WebSearch`
  queries aimed specifically at Paraguayan press retrospectives of the 1979
  title. That surfaced a name no prior pass had found - **Aldo Florentín**,
  who took the armband after regular captain Hugo Talavera was
  controversially benched by the federation president just before the
  series decider - confirmed by two independent, named sources (his own
  dedicated English Wikipedia biography, and Paraguay's ABC Color newspaper),
  clearing the same two-independent-source bar every other edition on this
  list already meets. One early search result suggested a conflicting
  narrative (that Julio César Romero, not Florentín, physically lifted the
  trophy); three follow-up passes with different phrasings never reproduced
  that claim while every other result kept crediting Florentín with both the
  armband and the presentation, so it was treated as an uncorroborated
  outlier rather than a genuine source conflict - the same judgment call the
  1987 edition's own entry already had to make explicit. Added a **1979**
  bullet to `content/copa-america.md`'s "Winning captains" section (between
  1975 and 1983, chronological order) and rewrote the section's intro bullet
  to describe all three resolved gaps (1983, 1987, 1979) instead of naming
  1979 as a standing exclusion - this closes the section completely, every
  edition from 1975 to 2024 now has a named, two-source-confirmed captain.
  Hand-translated into `hr/competitions/copa-america.astro`'s own `notes`
  array. `content/copa-america.md`'s `lastReviewed` bumped to 2026-09-05. New
  e2e coverage (one EN + one HR assertion) in `tests/e2e/mobile.spec.ts`. All
  700 PDFs regenerated and reverified clean (`pnpm build:pdfs` then `pnpm
  check:pdfs`, since this content edit and the `docs/SOURCES.md` addition
  both mark every PDF's shared References section stale, by design). Full
  standing health check clean: `pnpm lint` (0/0/0), `pnpm test` (unit tests
  unchanged - no `src/lib` logic touched), `pnpm build` (711 pages,
  unchanged), `check:links`/`check:sitemap`/`check:precache`/`check:perf`
  all clean, full cold-start `pnpm test:e2e` after the two new assertions.
  See `docs/PROJECT_STATUS.md`'s matching entry and `docs/SOURCES.md`'s
  matching entry (full citation list, including the outlier-claim caveat)
  for detail. **Left for a future pass:** the same environment-blocked items
  as every recent run (`typescript` 7, `docs/SOURCES.md` link-liveness), plus
  Nations League's Team of the Tournament for 2021/2023/2025 (unchanged, no
  new source lead - and now confirmed directly egress-blocked at the
  UEFA.com domain level, not just at Wikipedia). With every named captain gap
  across all four team competitions now closed, the next content-gap pass
...
- **`check:print-width`: a new full-site print-media width sweep**: closed
  2026-09-08 (eighty-third intensive run) - a standing health check first
  (`pnpm install`; `pnpm outdated` still shows only the blocked `typescript`
  7 entry, re-confirmed; `pnpm dlx knip --no-config-hints` matched the
  standing baseline; `pnpm lint` 0/0/0; `pnpm test` 542/542 unit; `pnpm build`
  ...
  run's baseline). Per this routine's own priority order, Copa América's
  captain gap is fully closed and Nations League's Team of the Tournament for
  2021/2023/2025 has now been re-confirmed unavailable across six consecutive
  prior runs with no new source lead - re-attempting either would just repeat
  a documented mistake, so this run took the eighty-second run's own closing
  suggestion and looked for another quality angle.
  `check:reflow`/`check:text-zoom` both widened their own stress axis (narrow
  viewport, then doubled text size) to a genuinely full-site sweep across the
  seventy-ninth through eighty-second runs, but the site's third stress axis -
  print media, the rendering path all 700 downloadable PDFs actually depend
  on - was still only covered by `tests/e2e/print-styles.spec.ts`'s
  hand-curated `WIDE_TABLE_PRINT_PAGES` list (18 pages: the six competition/
  award landing pages, `/records`, `/compare` and `/compare-players`, both
  languages). Confirmed via `grep -rl '<table' src/` that this list is in
  fact already exhaustive for the one bug class that motivated it (a real
  HTML `<table>` overflowing the printable A4-landscape content width) - no
  other `src/` file renders a raw `<table>` - so this wasn't a live bug the
  way the seventy-third run's original filter-select/table-width fixes were.
  What was still missing was the same "permanent full-site tool, not a fixed
  ...
  overflow at print width on any of the site's other ~700 pages (per-edition
  ...
  with nothing in place to catch it.
  ...
  `PRINT_CONTENT_WIDTH_PX` constant already uses, before the same
  ...
  `check-reflow.mjs`/`check-text-zoom.mjs` established. New unit test
  (`tests/unit/checkPrintWidth.test.ts`) pins the exported
  `PRINT_CONTENT_WIDTH_PX` constant to both its derivation and the literal
  1032px value, so it can't silently drift from `print-styles.spec.ts`'s own
  copy. Ran it against all 711 real content pages (both languages): **zero
  overflow found** - matches the hand-written spec's own clean result, no
  code fix needed this run, but now a permanent, reusable script like
  `check:lighthouse`/`check:reflow`/`check:text-zoom` so a future layout or
  content change that breaks print-width layout anywhere on the site gets
  caught. Not wired into `.github/workflows/ci.yml`, the same reasoning the
  other three full-site sweeps document (a ~700-page-load sweep is too slow
  for a required PR gate). No `content/*.md` or PDF-source file touched, so
  `check:pdfs` stayed clean at 700/700 throughout with no regeneration
  needed. Full standing health check clean: `pnpm lint` (0/0/0), `pnpm test`
  (543/543 unit, up from 542 - the one new pure-constant test), `pnpm build`
  (711 pages, unchanged), `check:links` (715 pages), `check:sitemap` (710
  entries), `check:precache` (37 URLs), `check:perf` (heaviest page still
  `hr/records`, 583.4 KB, unchanged), `check:pdfs` (700/700 fresh), the new
  `check:print-width` (711/711 pages clean), full cold-start `pnpm test:e2e`
  ...
  every recent run (`typescript` 7, `docs/SOURCES.md` link-liveness, Nations
  League's Team of the Tournament for 2021/2023/2025). With reflow,
  Lighthouse, text-zoom and now print-width all full-site clean, a future run
...
- **FIFA World Cup and UEFA EURO "Final attendance"**: closed 2026-09-11
  (ninety-eighth intensive run) - a standing health check first (`pnpm
  install`, `pnpm outdated` unchanged: only the blocked `typescript` 7 entry;
  `pnpm lint`/`test`/`build`/`check:links`/`check:sitemap`/`check:precache`/
  `check:perf`/`check:pdfs`/`check:jsonld`/`check:meta`/`check:html`/
  `check:spelling` all clean and a full cold-start `pnpm test:e2e`, 945/945
  passed, matching the ninety-seventh run's baseline). Extended the "Final
  attendance" angle - live for UEFA Nations League and Copa América since
  the ninety-fourth/ninety-sixth runs - to the two team-competition families
  it hadn't reached yet, this routine's own top content priority. Unlike
  Nations League's finals, World Cup and EURO finals are famous enough that
  genuinely independent corroboration (not just a Wikipedia figure
  reconfirmed by rephrasing a search) turned out to be available for almost
  every edition: two research passes (a broad first pass, then a second
  pass targeted specifically at every edition the first pass could only tie
  to Wikipedia, aimed at finding one more genuinely independent domain)
  cleared 21 of 23 World Cup editions and 15 of 17 EURO editions. Added a
  reported attendance figure to each cleared edition's existing "Final
  venues" bullet in `content/fifa-world-cup.md`/`content/uefa-euro.md` (not
  a new section - the same annotation-not-a-stub approach the ninety-fourth
  run's Nations League/Copa América work used) and hand-translated into
  `hr/competitions/world-cup.astro`'s and `hr/competitions/euro.astro`'s own
  `notes` arrays. Four editions were deliberately left out, for two
  different reasons: 1930 and 1950 (World Cup) are genuinely **disputed**
  rather than under-sourced - 1930 splits between an official 68,346 and
  contemporary press estimates around 90,000-100,000, and 1950's Maracanã
  ...
  1996 and 2020 (EURO) have one consistently repeated figure each but
  neither pass could tie it to a first-tier independent outlet, the same
  single-source caution that already excludes Nations League's
  2021/2023/2025 figures. 1986's 114,600 is also flagged as the largest
  attendance of any World Cup final (independently corroborated by Guinness
  World Records), and 1968's EURO final replay's 32,886 is presented
  alongside the original drawn match's own separately reported 68,817.
  `lastReviewed` bumped to 2026-09-11 on both content files. New e2e
  coverage (EN + HR) spot-checks one confirmed and one deliberately-excluded
  edition per page. All 700 PDFs regenerated and reverified clean. Full
  standing health check clean again after the edit: `pnpm lint` (0/0/0),
  `pnpm test` (588/588 unit, unchanged - presentation-layer content, no new
  unit-testable logic), `pnpm build` (711 pages), `check:links` (715
  pages), `check:sitemap` (710 entries), `check:precache` (37 URLs),
  `check:perf` (heaviest page `hr/records` 600.5 KB, within the 610 KB
  budget), `check:pdfs` (700/700 fresh), `check:jsonld` (1,783/1,783 blocks
  valid), `check:meta` (710/710 clean), `check:html` (711/711 valid),
  `check:spelling` (0 issues - no new proper nouns, only numbers), plus a
  full cold-start `pnpm test:e2e`. See `docs/PROJECT_STATUS.md`'s matching
  entry for detail. **Left for a future pass:** the same environment-blocked
  items as every recent run (`typescript` 7, `docs/SOURCES.md`
  link-liveness, Nations League's Team of the Tournament for
  2021/2023/2025, the `long-title` brand-suffix decision), plus the
  Nations League 2023 attendance conflict, 2021/2025's still-unconfirmed
  Nations League figures, and now also EURO 1996/2020's and World Cup
  1930/1950's excluded attendance figures as specific, named gaps rather
  than an unconfirmed blank spot to keep re-researching from scratch. A
...
- **`check:edition-header-labels`: new permanent check for the seven
  per-family edition-page `HEADER_LABELS` translation maps, plus one real
  dead-key bug it caught**: closed 2026-09-13 (hundred-and-fourteenth
  intensive run) - a standing health check first (all clean, matching the
  hundred-and-thirteenth run's baseline; `pnpm outdated` still only the
  blocked `typescript` 7 entry). Extended this run's own established
  "read a shared component plus every call site end to end" method to the
  one area explicitly flagged as not yet swept this way: the seven
  `src/pages/hr/competitions/**/[year].astro` route trees (World Cup, EURO,
  Nations League, Copa América, Ballon d'Or, Golden Boot x2), each of which
  hand-maintains its own `HEADER_LABELS` object translating that
  competition's raw English column headers for `EditionView.astro`'s
  `label()` helper - which silently falls back to the raw English header
  when a key is missing, the exact same "quiet fallback" shape that produced
  the `hr/records` unit-noun bug (hundred-and-fifth run) and the Croatian
  References `noteText` bug (hundred-and-twelfth run). Cross-checked all
  seven maps against their live source table headers in `content/*.md`: six
  were already complete, but UEFA Nations League's map carried a dead
  `Season: 'Sezona'` entry - harmless today only because `EditionProfile`
  deliberately excludes the Year/Season column from `facts` (so `label()`
  never looks it up), but a maintenance trap masquerading as a real
  translation. Removed it. New permanent tool,
  `scripts/check-edition-header-labels.mjs`
  (`pnpm check:edition-header-labels`): parses each family's source table
  header row and its `.astro` file's `HEADER_LABELS` keys and flags both a
  missing translation and a stale key with no matching column - plain
  regex/string parsing of `content/*.md` plus the seven route files, no
  build or browser needed, so wired into `.github/workflows/ci.yml` as a
  required PR gate right after `check:award-tallies`. 4 new unit tests for
  the exported `parseHeaderLabelKeys()` helper. All 8 affected PDFs
  (`edition-nations-league-<slug>[-hr].pdf`, EN+HR x4 seasons) regenerated
  and reverified with `pnpm check:pdfs` (700/700 fresh) - the dead key's
  removal doesn't change any rendered `<dt>` text, but it does change the
  source file `check:pdfs` hashes against. Full standing health check clean
  after the change: `pnpm lint` (190 files, 0/0/0), `pnpm test` (635/635
  ...
  "read end to end" method has now covered `src/lib/`, `src/components/`,
  the top-level EN/HR page pairs, and the seven per-family edition-page
...
- **Dependency patch bump (html-validate 11.15.0 -> 11.16.0) plus a manual
  ...
  **Left for a future pass:** the same environment-blocked items as ever
  ...
  pass.
- **Dependency patch bump (cspell 10.3.2 -> 10.3.3) plus the
  hundred-and-thirty-first run's own suggested next step - extending the
  manual-walkthrough method to the desktop "More" overflow menu - finds and
  fixes a real bug: `#nav-more-menu` rendered up to 156px past the
  viewport's right edge at real, reachable desktop widths**: closed
  2026-09-16 (hundred-and-thirty-second intensive run) - a standing health
  check first (`pnpm install --frozen-lockfile`, `pnpm outdated` found one
  in-range patch bump beyond the still-blocked `typescript` 7 entry -
  installed cleanly; full lint/unit/build and all eighteen `check:*`
  scripts clean, matching the hundred-and-thirty-first run's own baseline:
  703/703 unit, 711 pages).
  ...
  that range - real, reachable windowed-browser widths, not a hypothetical
  extreme - it sits close enough to the right edge that the menu's own
  right side renders past the viewport. A width sweep (960px to 1400px in
  5px steps, both languages, via a throwaway Playwright script rendering
  the live `astro preview` output) found the menu overflowing the right
  edge at 965-1025px in English and a wider 1050-1145px band in Croatian
  (longer link labels shift the row's own fit further out) - up to 156px
  of overflow at the worst point. Since `body` has `overflow-x: hidden`
  (`global.css`, to stop the page scrolling sideways on small screens),
  that overflow was clipped silently with no scrollbar: the menu's own
  rightmost links (in the worst cases, every link in the menu, since they
  all share one `x` position stacked in a column) were simply unreachable
  by a mouse/touch reader. Confirmed directly, not just by the coordinate
  sweep: at 1000px, clicking the last link before the fix threw off the
  end of a normal (non-forced) Playwright click; after the fix the same
  click lands. Invisible to every existing check for the reason the
  hundred-and-thirty-first run's own entry already established for the
  drawer bug: none of them measure an *open dropdown's* own bounding box
  against the viewport, only the document's overall `scrollWidth` (and
  `check:reflow` only ever runs at 320px, well below the `>=60rem`
  breakpoint this menu exists at).

  A static fix isn't enough on its own, and this run tried the obvious one
  first before rejecting it: right-aligning the menu instead
  (`inset-inline-end: 0`, flush to the toggle's trailing edge) cuts the
  overflow instances roughly in half across the same sweep, but doesn't
  eliminate them - it just moves the bug to the *left* edge in a narrower
  band (960-1045px) where a separate, pre-existing edge case (the nav row
  itself can still wrap onto two lines right at the exact `60rem`
  breakpoint, a sub-pixel rounding gap between the CSS breakpoint and the
  viewport's real available width once a scrollbar is subtracted) strands
  the toggle alone at the row's own *left* edge instead of its usual
  rightmost position - from there a right-aligned menu overshoots left.
  Fixed properly with a small measure-and-clamp in the same `setOpen()`
  that already toggles `hidden`/`aria-expanded` (`Nav.astro`'s
  `initNavMore`): on every open, read the menu's real
  `getBoundingClientRect()` against `document.documentElement.clientWidth`
  and set a `--nav-more-shift` custom property consumed by a
  `transform: translateX(var(--nav-more-shift, 0px))` on the menu (CSS
  keeps the original `inset-inline-start: 0` as the base position); shifts
  left if the menu overflows right, right if it overflows left, zero
  otherwise. Physical `translateX`, not a logical property, since the
  measurement itself is always physical/LTR pixels and both site languages
  render left-to-right anyway. Re-ran the same 960-1400px x 5px x both-
  language sweep after the fix: **zero overflow instances**, versus 33
  before the fix and 17 with the rejected right-align-only approach - the
  only anchor that's correct regardless of which edge the toggle ends up
  near.

  New regression coverage: one e2e test in the existing `desktop nav
  "More" menu (>=60rem)` describe block (`tests/e2e/mobile.spec.ts`),
  opening the menu at a real 1000px width and asserting every link's
  `getBoundingClientRect()` stays within `[0, 1000]`, then actually
  clicking the previously-unreachable last link ("Sources") through to
  ...
  re-ran the full
  existing `tests/e2e/mobile.spec.ts` suite standalone (329/329, up from
  328/328 by exactly the one new test) since the fix touches the same file
  those tests already cover end-to-end - clean, no regression.

  No content file touched, so no PDF regeneration was needed. Full
  standing health check clean after the change: `pnpm lint` (0/0/0),
  `pnpm test` (703/703, unchanged - a CSS/client-script-only fix has no
  unit-testable `.ts` logic), `pnpm build` (711 pages, unchanged), all
  eighteen `check:*` scripts clean (re-run in full, including
  `check:reflow`/`check:text-zoom`/`check:print-width`, none of which are
  sensitive to this defect class for the same reason the
  hundred-and-thirty-first run's entry already gives). A full cold-start
  ...
  - it passed cleanly re-run in isolation) and then a stalled run that had
  to be killed. Substituted the full standalone `tests/e2e/mobile.spec.ts`
  file (329 tests - every header/drawer/focus-trap/nav-more/compare-panel
  test the site has, i.e. everything that actually exercises `Nav.astro`)
  once load had settled: clean, **329/329 passed, 1.8 minutes**. A future
  run should still get a full cold-start `pnpm test:e2e` confirmation once
  the environment isn't under the same contention - this run's own targeted
  substitute is strong evidence, not a full replacement.
  ...
  **Left for a future pass:** the same environment-blocked items as ever
  ...
  cold-start `pnpm test:e2e` confirmation once the sandbox isn't under
  heavy CPU contention, and the pre-existing "nav row wraps onto two lines
  right at the exact `60rem` breakpoint" edge case noted above - harmless
  now that the dropdown clamp handles it, but the row-wrap itself is still
  there and was never itself the target of a fix. The manual-walkthrough
  method has now found a real, user-facing bug in seven consecutive runs -
  a future pass could extend it next to the "More" menu's own focus
...
- **Mouse-drag/touch-drag interaction sweep of every click-driven disclosure
  widget**: closed 2026-09-17 (hundred-and-thirty-sixth intensive run) - a
  standing health check first (`pnpm install --frozen-lockfile`; `pnpm
  outdated` still only the blocked `typescript` 7 entry; `pnpm lint`
  (0/0/0), `pnpm test` (703/703 unit), `pnpm build` (711 pages), all
  eighteen `check:*` scripts including the three browser-based sweeps with
  `PW_EXECUTABLE_PATH` set, and `pnpm dlx knip --no-config-hints` - all
  clean, matching the hundred-and-thirty-fifth run's own baseline exactly).
  A fresh `pnpm outdated` turned up nothing new to bump. Also confirmed
  `WebFetch`/`curl` egress to `en.wikipedia.org` is still blocked in this
  environment (`EGRESS_BLOCKED`), same as every prior attempt, and a
  `WebSearch` re-check of the Nations League 2023 final attendance still
  only reproduces the same Wikipedia figure (41,110) already on record
  alongside RFEF's conflicting 41,500 - no new source, conflict stays open.

  Picked up the hundred-and-thirty-fifth run's own closing suggestion:
  every open/close/navigate action on the site's four click-driven
  disclosure widgets (the mobile `#site-menu` drawer, the desktop
  `#nav-more-menu` "More" overflow menu, and both "find a team"/"find a
  player" search comboboxes) is wired to a `click` listener rather than
  `pointerdown`/`touchstart` - deliberately, so a press-drag-release
  gesture (scrolling the drawer, selecting text in a search input, a mouse
  drag that starts on one control and releases on another) never fires it,
  since browsers only dispatch `click` when mousedown/mouseup or a touch
  tap land on the same interactive target. That invariant had never
  actually been exercised with a real drag gesture - every existing
  menu/combobox test in `tests/e2e/` drives a plain `.click()`, which
  can't distinguish a genuine click from a click-terminated drag.

  Rendered the real built site with Playwright/Chromium
  (`/opt/pw-browsers/chromium`) and drove five drag scenarios with actual
  `mouse.down()`/`mouse.move({ steps })`/`mouse.up()` sequences: dragging
  over a nav link inside the open mobile drawer (scrolls/selects, drawer
  stays open, no navigation); a drag starting on the desktop "More" toggle
  and releasing 400px away (menu stays closed); the reverse, a drag
  starting elsewhere and releasing on the toggle (menu stays closed); and
  a drag from the team-search input into a visible result option (no
  navigation, text-selection only). All five confirmed the expected
  browser-native behaviour - a genuine negative result, no bug found. New
  permanent regression coverage added anyway
  (`tests/e2e/drag-interactions.spec.ts`, 4 tests) so a future refactor
  ...
  **967/967 passed, 12.4 minutes** (up from 963/963 by exactly the four new
  ...
  touched neither, so no drift to catch either).

  **Left for a future pass:** the same environment-blocked items as ever
  (`typescript` 7, `docs/SOURCES.md` link-liveness, the `long-title`
  brand-suffix decision, the Nations League Team of the Tournament sourcing
  question - confirmed exhausted), the Nations League 2023 attendance
  conflict (41,110 vs. 41,500 - a documented disagreement, not an
  unconfirmed figure to keep re-searching for), 2021/2025's still-unconfirmed
  ...
  hyphenation-rendering visual re-check. With the drag-interaction angle now
  also closed out as a genuine negative result, a future pass's best bet is
...
- **New e2e coverage for a downloaded PDF's own internal link, opened while
  offline**: closed 2026-09-20 (hundred-and-fifty-third intensive run) - the
  hundred-and-fifty-first run's own remaining suggested angle. A standing
  health check first (`pnpm install --frozen-lockfile`; `pnpm outdated`
  still only the blocked `typescript` 7 entry; `pnpm lint` 0/0/1; `pnpm test`
  734/734; `pnpm build` 711 pages; all seventeen fast `check:*` scripts
  clean - all matching the hundred-and-fifty-second run's baseline).
  Investigated the mechanism before writing anything: every downloadable
  PDF's internal team/player links (`scripts/generate-pdfs.mjs`'s
  `rewriteInternalLinksForPrint()`, hundred-and-forty-eighth run) are plain
  production URLs with no trailing slash - the exact same slash-less form
  every in-app link built with `withBase()` already uses - and `sw.js.ts`'s
  navigate handler normalizes every navigation request's cache key with a
  trailing slash before reading or writing Cache Storage (the
  hundred-and-fifty-first run's own fix), regardless of how the browser
  arrived at that URL. So a PDF's own internal link is not actually a
  distinct code path from an ordinary in-app link click - but no existing
  test had exercised it for a *team/player profile page* specifically: every
  offline test in `mobile.spec.ts` so far only used top-level nav pages
  (`/records`, `/competitions/world-cup`, etc.), which are precached on
  install (`NAV_LINKS`/`PRECACHE_URLS`, `src/lib/routes.ts`). Team and player
  pages carry no precache entry of their own - they only ever land in Cache
  Storage opportunistically, the first time they're actually visited online
  - so this is a genuinely different, previously-untested case: does the
  ordinary network-first/cache-fallback fetch handler correctly serve a
  *leaf* page (the kind the vast majority of a PDF's 409,320 link
  annotations actually point to) once it's been visited, not just the
  nineteen nav pages.
  Added one new test to the `Installability and offline reading` describe
  block in `tests/e2e/mobile.spec.ts`: visits `/teams/brazil` online (the
  same way a reader would before ever downloading its PDF), goes offline,
  then re-requests the exact same slash-less URL as a fresh navigation
  (rather than a `page.reload()`) - the same request shape as a click
  arriving from outside the app entirely (a PDF viewer, another tab) - and
  asserts the cached content still renders.
  Verified it is real coverage, not a tautology, the same way the
  hundred-and-fifty-second run's own activate-handler test was: temporarily
  dropped the `cache.put()` call from `sw.js.ts`'s navigate handler, killed
  the running preview daemon and forced a fresh `pnpm build` (a stale,
  reused preview server from before the edit was caught as a false pass on
  a first attempt - confirmed by checking `dist/sw.js` still carried the
  unmodified source, not by trusting the test result blindly), and
  confirmed the new test then failed with the home page's `<h1>` showing
  instead of Brazil's. A first attempt to prove this a different way -
  disabling Chromium's own HTTP cache via a page-level CDP
  `Network.setCacheDisabled` session - made no difference either way,
  consistent with `pwa-savedata.spec.ts`'s own documented finding that a
  page-level CDP override doesn't reach the service worker's own separate
  execution context; the working verification method (breaking the source,
  confirming a real preview-daemon rebuild, re-testing) didn't need it.
  Restored the original `sw.js.ts` (confirmed `git diff` empty) and
  reconfirmed the test passes again. Full standing health check clean after
  adding the test: `pnpm lint` (0/0/1, unchanged), `pnpm test` (734/734,
  unchanged - e2e-only coverage of already-generated `sw.js.ts` behavior),
  `pnpm build` (711 pages, unchanged), every fast `check:*` script clean,
  the four daemon-driven ones (`check:reflow`/`check:text-zoom`/
  `check:print-width`/`check:lighthouse`) clean, and a full cold-start `pnpm
  test:e2e` (confirmed no stale preview server via `ps aux` beforehand):
  **1016/1016 passed** (16.0 minutes), up from 1015 - the one new test, no
  regressions anywhere else in the suite. No content file, PDF-source
  component, or `src/` file changed, so no PDF regeneration was needed.
  **Left for a future pass:** the same environment-blocked items as every
  recent run (`typescript` 7, `docs/SOURCES.md` link-liveness, the
  `long-title` brand-suffix decision, the Nations League Team of the
  Tournament sourcing question - confirmed exhausted), the Nations League
  ...
  hundred-and-fifty-first run's own suggested angles now closed, a future
  pass likely needs either a fresh dependency-upgrade attempt, the
